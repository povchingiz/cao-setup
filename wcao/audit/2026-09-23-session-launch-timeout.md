...
| 3 | `LOCAL_API_KEY` is defined nowhere — not in the environment, `~/.bashrc`, `~/.profile`, or tmux global env. `~/.aws/opencode/opencode.json` resolves `apiKey: "{env:LOCAL_API_KEY}"`, and `cao-run` propagates it only under `if [ -n "$LOCAL_API_KEY" ]`, so it is skipped silently. Any `opencode_cli` worker on the `nitec` provider will fail authentication. Needs the operator's key. | High | operator |
